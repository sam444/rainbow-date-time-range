...
export default class PageFooter extends React.Component {
	...
	render() {
		...
		let _config = sessionStorage.getItem("packageJson")? JSON.parse(sessionStorage.getItem("packageJson")):null;
		if(!_config){
				_config = JSONUtil.getJSON("package.json");
				sessionStorage.setItem("packageJson",JSON.stringify(_config));
		} 
		const tip = [];
		if(_config!=null){
			tip.push("<table>");		
